...
df = pd.concat(dfs) # concatenates into a single dataframe

df["datetime"] = pd.to_datetime(df[["year", "month", "day", "hour"]]) # constructs a new column called datetime
df = df[df["datetime"] < "2014-03-01"] # filters the dataframe to have one exact year
...
def haversine(station_one, station_two, station_coords):
    ...
    long_onerad = cp.deg2rad(long_one)
    lat_onerad = cp.deg2rad(lat_one)
    long_tworad = cp.deg2rad(long_two)
    lat_tworad = cp.deg2rad(lat_two)

    dist_long = long_tworad - long_onerad
    dist_lat = lat_tworad - lat_onerad

    a = cp.sin(dist_lat/2)**2 + (cp.cos(lat_onerad) * cp.cos(lat_tworad)  * cp.sin(dist_long/2)**2)
    d = (2 * radius) * cp.arcsin(cp.sqrt(a))

    return d

# ----- Check for missing values -----
bool_df = pd.isnull(pm25["PM2.5"])
print(pm25[bool_df].to_string())

# ----- Computing the distance matrix -----
stations = sorted(pm25["station"].unqiue())
d_station = cp.zeros((len(stations), len(stations)))

for i in range(len(stations)):
    for j in range(i + 1, len(stations)):
        distance = haversine(stations[i], stations[i], station_coords)
