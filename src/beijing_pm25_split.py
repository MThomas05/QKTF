...
# ----- Tensor construction (station x hour x day) -----
...
index = pd.MultiIndex.from_product([stations, hours, days], names=["stations", "hours", "days"])
pm25_index = (pm25.set_index(["station", "hour", "day"])["PM2.5"].reindex(index))

I_numpy = pm25_index.to_numpy().reshape(len(stations), len(hours), len(days))
...
if __name__ == "__main__":
    ...
    # ----- Development save -----
    np.savez(path, I_train=I_train, mask_train=mask_train, **metadata)
